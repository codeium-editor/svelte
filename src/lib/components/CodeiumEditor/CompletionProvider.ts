...
import { Document as DocumentInfo, GetCompletionsResponse, CompletionItem } from '$lib/api/proto/exa/language_server_pb/language_server_pb'
import { LanguageServerService } from '$lib/api/proto/exa/language_server_pb/language_server_connect';
import { Language, Metadata } from '$lib/api/proto/exa/codeium_common_pb/codeium_common_pb';

import { numUtf8BytesToNumCodeUnits,
    numCodeUnitsToNumUtf8Bytes } from '$lib/utils/utf'
...
export default class MonacoCompletionProvider {
    ...
    public async provideInlineCompletions(
    ...
    ): Promise< monaco.languages.InlineCompletions<MonacoInlineCompletion> | undefined> {
        ...
        const inlineCompletionItems = completionItems.map((completionItem) => this.createInlineCompletionItem(completionItem, document)).filter((item?: MonacoInlineCompletion): item is MonacoInlineCompletion => !!item);

        this.status = Status.SUCCESS;
        this.message = inlineCompletionItems.length === 1 ? "Generated 1 completion" : `Generated ${inlineCompletionItems.length} completions`;
     ...
     */
    private getDocumentInfo(document: Document, position: Position): DocumentInfo {
     ...
     */
    private createInlineCompletionItem(completionItem: CompletionItem, document: Document): MonacoInlineCompletion | undefined {
