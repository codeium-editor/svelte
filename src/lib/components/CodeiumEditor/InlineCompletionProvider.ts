...
export default class InlineCompletionProvider implements monaco.languages.InlineCompletionsProvider<monaco.languages.InlineCompletions<MonacoInlineCompletion>> {
    private numCompletionsProvided: number;
    private completionCount: number;
    readonly completionProvider: MonacoCompletionProvider;

    constructor(
        grpcClient: PromiseClient<typeof LanguageServerService>, completionCount: number,
        codeiumStatus: Status,
        codeiumStatusMessage: string
    ) {
        this.numCompletionsProvided = 0;
        this.completionCount = completionCount;
        this.completionProvider = new MonacoCompletionProvider(grpcClient, codeiumStatus, codeiumStatusMessage);
    }
    ...
    async provideInlineCompletions(
        ...
        const completions = await this.completionProvider.provideInlineCompletions(model, position, token);
