<script lang="ts">
  ...
  export let onEditorMount: (editor: Monaco.editor.IStandaloneCodeEditor, monaco: typeof Monaco) => void = () => {};
  ...
  onMount(async () => {
      ...
      const inlineCompletionsProvider = new InlineCompletionProvider(grpcClient, completionCount, codeiumStatus, codeiumStatusMessage);

      monaco.languages.registerInlineCompletionsProvider({ pattern: "**"}, inlineCompletionsProvider);

      monaco.editor.registerCommand("codeium.acceptCompletion", (_: unknown, completionId: string, insertText: string) => {
